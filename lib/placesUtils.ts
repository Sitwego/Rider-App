...
function getAddressComponents(
  ...
  addressComponents.forEach((addressComponent) => {
    addressComponent.types.forEach((addressType) => {
      if (
        !(addressType in fieldsToExtract) ||
        !(addressType in result && result[addressType] === "")
      ) {
        return;
      }
...
type GetPlaceAutocompleteTermsResultKey =
  | "country"
  | "state"
  | "city"
  | "street"
  | "streetNumber";
type GetPlaceAutocompleteTermsResult = Partial<
  Record<GetPlaceAutocompleteTermsResultKey, string>
>;
 ...
 */
function getPlaceAutocompleteTerms(
  addressTerms: AddressTerm[],
): GetPlaceAutocompleteTermsResult {
  ...
  fieldsToExtract.forEach((fieldToExtract, index) => {
    ...
    result[fieldToExtract] =
      fieldTermIndex >= 0 ? addressTerms.at(fieldTermIndex)?.value : "";
  });
  return result;
}
